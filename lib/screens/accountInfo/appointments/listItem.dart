...
import 'package:doctor_consultation/screens/accountInfo/appointments/moreOptions.dart';
...
///
class AppointmentsListItem extends StatefulWidget {
  const AppointmentsListItem({Key key, this.appointment}) : super(key: key);
  final Appointment appointment;

  @override
  _AppointmentsListItemState createState() => _AppointmentsListItemState();
}

class _AppointmentsListItemState extends State<AppointmentsListItem>
  ...
  void handleChange(DocumentSnapshot event) {
    log('Appointment updates listening for id ${event.id}');
    setState(() {
      //TODO:
      isActive = true;
      // event.data()['isDoctorActive'];
    });
  }
...
class BuildItem extends StatelessWidget {
  const BuildItem({Key key, this.appointment, this.isActive = false})
      : super(key: key);
  ...
  void moreOptions(BuildContext context, Appointment appointment) {
    if (appointment.status == AppointmentStatusType.SCHEDULED) {
      ...
      UiController.modalBottomSheet(context,
          child: MoreOptions(
...
class PreviousAppointmentItem extends StatelessWidget {
  const PreviousAppointmentItem({Key key, this.appointment}) : super(key: key);
