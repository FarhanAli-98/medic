// Copyright (c) 2020 Aniket Malik [aniketmalikwork@gmail.com]
// All Rights Reserved.
//
// NOTICE: All information contained herein is, and remains the
// property of Aniket Malik. The intellectual and technical concepts
// contained herein are proprietary to Aniket Malik and are protected
// by trade secret or copyright law.
//
// Dissemination of this information or reproduction of this material
// is strictly forbidden unless prior written permission is obtained from
// Aniket Malik.

import 'dart:developer';

import 'package:firebase_messaging/firebase_messaging.dart';

///
/// ## `Description`
///
/// Takes care of the notification in the local scope.
/// Handle the incoming notification data and reacts to it
/// based on the type of data is provided.
///
class NotificationController {
  static bool _isNotificationClicked = false;

  bool get isNotificationClicked => _isNotificationClicked;

  /// Set the notification clicked flag to check for notification click
  /// on splash screen onLaunch event
  void setNotificationClicked(bool value) {
    log('setting notification clicked with value $value',
        name: 'NotificationController');
    _isNotificationClicked = value;
  }

  static RemoteMessage _notificationObject;

  RemoteMessage get notificationObject => _notificationObject;

  void setNotificationObject(RemoteMessage message) {
    _notificationObject = message;
  }
}
